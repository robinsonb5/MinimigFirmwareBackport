...
const char *config_cpu_speed[] = {"7.09 MHz ", "14.18 MHz", "49.63MHz"};
...
enum HelpText_Message {HELPTEXT_NONE,HELPTEXT_MAIN,HELPTEXT_HARDFILE,HELPTEXT_CHIPSET,HELPTEXT_MEMORY,HELPTEXT_VIDEO,HELPTEXT_ABOUT};
...
#ifdef MINIMIG_V1_0
	"                                Processor 68000 can run in NORMAL mode at a frequency of 7.09 MHz or in TURBO mode at a frequency of 49 MHz.",
#else
...
#else
#ifdef MINIMIG_V1_0
	"                                Minimig can make use of up to 2 megabytes of Chip RAM, up to 1.5 megabytes of Slow RAM (A500 Trapdoor RAM).  To use the Action Replay feature you will need an Action Replay 3 ROM file on the SD card, named AR3.ROM.  You will also need to set Slow RAM to no more than 1.5 megabytes.",
...
"                                 Minimig by Dennis van Weeren.  Chipset improvements by Jakub Bednarski and Sascha Budgenbach.  Menu / disk code by Dennis van Weeren, Jakub Bednarski and Alastair M. Robinson.  Backport the new Menu from the project 'Minimig for the DE1 board' by Miloslav Martinka.  Build process, repository and tooling by Christian Vogelgsang.  Minimig logo based on a design by Loriano Pagni.  Minimig is distributed under the terms of the GNU General Public License version 3.",
	0
};
...
void HandleUI(void)
{
	...
	case MENU_ADVANCED1 :
      helptext=helptexts[HELPTEXT_MAIN];
      menumask=0x07;	// Fast SPI
      OsdSetTitle("Advanced",OSD_ARROW_LEFT|OSD_ARROW_RIGHT);
      
      OsdWrite(0, "", 0,0);
      OsdWrite(1, "       Load FPGA core", menusub == 0,0);
      OsdWrite(2, "", 0,0);
      OsdWrite(3, "       Fast SPI", menusub == 1,0);
      OsdWrite(4, "", 0,0);
      OsdWrite(5, "", 0,0);
      OsdWrite(6, "", 0,0);
    ...
    case MENU_ADVANCED2 :
        ...
        {
            if (menusub == 0)	// Load FPGA core
            {
                SelectFile("BIN", SCAN_DIR | SCAN_LFN, MENU_COREFILE_SELECTED, MENU_MAIN1);
//				menusub=0;
			}
            if (menusub == 1)	// SPI enable
        	{
  			if (GetSPIMode() == SPIMODE_FAST) { 
				menustate=MENU_SPI_ENABLE1;
				menusub=1; }
			else {
				menustate=MENU_SPI_ENABLE1;
				menusub=0; }
	...
	case MENU_COREFILE_SELECTED :
         if (iCurrentDirectory) // if not root directory
	     ...
	     else
        	{
        	 altcore_on = 1;
	    	 ConfigureFpga();
             printf("name alt core: %s\r", altcore);
		  }
	     PullUpCard();
	     WaitTimer(5000);
	     BootExit();
    ...
    case MENU_MISC2 :
        ...
        if (menu)
            menusub=0, menustate = MENU_NONE1;
		if (left)
			menusub=0, menustate = MENU_ADVANCED1;
        else if (select)
        {
            if (menusub == 0)	// Reset
            {
	...
	case MENU_ABOUT1 :
		helptext=helptexts[HELPTEXT_ABOUT];
...
//        strcat(s, config_cpu_speed[config.cpu & 0x03]); 
...
*/
        ...
        if (select)
        {
            ...
            else if (menusub == 3)
            {
				if(config.hardfile[1].enabled==HDF_FILE)
				{
				    ...
				    printf("config.hardfile[1].enabled: %s\r", config.hardfile[1].enabled);
				}
				else if(config.hardfile[1].enabled==(HDF_FILE|HDF_SYNTHRDB))
				{
					config.hardfile[1].enabled&=~HDF_SYNTHRDB;
					config.hardfile[1].enabled +=1;
					printf("config.hardfile[1].enabled: %x\r", config.hardfile[1].enabled);
				}
				else
				{
					config.hardfile[1].enabled +=1;
    ...
    case MENU_HARDFILE_EXIT :
         ...
         {
        	if ((config.hardfile[0].enabled != 0) && (config.hardfile[1].enabled == 0))
       		{
				switch (config.hardfile[0].enabled)
					{
					case HDF_CARD:
						 menustate = MENU_WARNIG_HARD_EXIT;
						 menusub = 0;
						break;
					case HDF_CARDPART0:
						 menustate = MENU_WARNIG_HARD_EXIT;
         	...
         	{
				switch (config.hardfile[1].enabled)
					{
					case HDF_CARD:
						 menustate = MENU_WARNIG_HARD_EXIT;
						 menusub = 0;
    ...
    case MENU_WARNIG_HARD_EXIT :

		menumask=0x01;
		parentstate=menustate;
 		OsdSetTitle("Warning",0);
        OsdWrite(0, "", 0,0);
        OsdWrite(1, "    Do NOT use setting", 0,0);
        OsdWrite(2, "    >partition1/2/3/4<", 0,0);
        OsdWrite(3, "    it can damage your", 0,0);
        OsdWrite(4, "     data on sd-card!", 0,0);
        OsdWrite(5, "", 0,0);
        OsdWrite(6, "", 0,0);
        OsdWrite(7, "             OK", menusub == 0,0);

        menustate = MENU_WARNIG_HARD_EXIT2;
        break;

    case MENU_WARNIG_HARD_EXIT2 :
        if (select || menu)
        {
            if (menusub == 0) // OK
		        menustate = MENU_HARDFILE_CHANGED1;
...
//						menustate = MENU_SYNTHRDB2_1;
    ...
    case MENU_ROMFILE_SELECTED2 :
        ...
        /* firmware menu */
        /******************************************************************/
     case MENU_SPI_ENABLE1 :
        helptext=helptexts[HELPTEXT_NONE];
        if (parentstate!=menustate)
        {
			menumask=0x03;
			if (GetSPIMode() == SPIMODE_FAST) menumask=0x02;
        }
   		parentstate=menustate;
    OsdSetTitle("FastSPI",0);
    
    OsdWrite(0, "", 0, 0);
	OsdWrite(1, "", 0, 0);
    if (GetSPIMode() == SPIMODE_FAST) {
    	OsdWrite(2, "     Fast SPI enabled", menusub == 0,1); }
    else {
   	   	OsdWrite(2, "     Enable Fast SPI ", menusub == 0,0); }
   	   
	OsdWrite(3, "", 0, 0);
	  sprintf(s,"    Info: spiclk %u MHz", spiclk);
	OsdWrite(4, s, 0, 0);
	OsdWrite(5, "", 0, 0);
	OsdWrite(6, "", 0, 0);
    OsdWrite(7, STD_EXIT, menusub == 1,0);
	
        menustate = MENU_SPI_ENABLE2;
        break;

    case MENU_SPI_ENABLE2 :
        if (menu)
        {
            menusub = 0;
            menustate = MENU_ADVANCED1;
    ...
    case MENU_FIRMWARE_OPTIONS_ENABLE1 :
		...
		OsdWrite(0, "", 0, 0);
        OsdWrite(1, "  Enabling fast SPI without", 0,0);
        OsdWrite(2, "    hardware modification", 0,0);
        OsdWrite(3, "       will be fatal!", 0,0);
        OsdWrite(4, "          Enable?", 0,0);
		OsdWrite(5, "", 0, 0);
        OsdWrite(6, "            yes", menusub == 0,0);
        OsdWrite(7, "            no", menusub == 1,0);
    ...
    case MENU_FIRMWARE_OPTIONS_ENABLED1 :
		...
		OsdWrite(0, "", 0, 0);
        OsdWrite(1, "   Fast SPI mode will be", 0,0);
        OsdWrite(2, "   activated after restart.", 0,0);
        OsdWrite(3, "   Hold MENU button while", 0,0);
        OsdWrite(4, "   powering-on to disable it.", 0,0);
		OsdWrite(5, "", 0, 0);
		OsdWrite(6, "", 0, 0);
        OsdWrite(7, "              OK", menusub == 0,0);
...
void ErrorFDDMessage(const char *message, unsigned char code)
{
    ...
    {
//        OsdClear();
 		OsdSetTitle("ErrorFDD",0);
        OsdEnable(0); // do not disable KEYBOARD
    }
        OsdWrite(0, "      *** ERROR FDD ***", 1,0);
	    OsdWrite(1, "", 0,0);
        strncpy(s, message, 32);
        s[32] = 0;
        OsdWrite(2, s, 0,0);
