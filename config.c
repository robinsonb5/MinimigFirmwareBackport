...
char UploadKickstart(char *name)
{
    ...
    if (RAOpen(&romfile, filename))
    {
        ...
        { // 512KB Kickstart ROM
            ...
            PrepareBootUpload(0xF8, 0x00, 0x08, 0x00);
			SendFile(&romfile);
            return(1);
        }
        ...
        { // 256KB Kickstart ROM
            ...
            PrepareBootUpload(0xF8, 0x00, 0x04, 0x00);
			SendFile(&romfile);
            return(1);
        }
        ...
        else if (romfile.size == 0x4000)
        { // 16 KB ROM
            BootPrint("Uploading 16 KB rom...");
            PrepareBootUpload(0xFD, 0x00, 0x00, 0x40);
			SendFile(&romfile);
            return(1);
        }
        else if (romfile.size == 0x8000)
        { // 32 KB ROM
            BootPrint("Uploading 32 KB rom...");
            PrepareBootUpload(0xFD, 0x00, 0x00, 0x80);
			SendFile(&romfile);
            return(1);
        }
        else if (romfile.size)
        { // 64 KB ROM
            BootPrint("Uploading ??64 KB rom...");
            PrepareBootUpload(0xFD, 0x00, 0x01, 0x00);
			SendFile(&romfile);
            return(1);
        }
        else
...
char UploadMacRom(char *name)
{
    ...
    if (RAOpen(&romfile,filename))
    {
        if (romfile.size == 0x20000)
        { // 138KB Kickstart ROM
            BootPrint("Uploading 132 KB MacPlus.rom...");
            ClearMemory(0x220000, 0x200000);
            PrepareBootUpload(0x22, 0x00, 0x02, 0x00);
			SendFile(&romfile);
			return(1);
        }
        else
        {
            BootPrint("\nUnsupported MACPLUS.ROM file size!!!");
			/* FatalError(6); */
			return(0);
        }
    }
...
char UploadActionReplay()
{
    if (RAOpen(&romfile, "AR3     ROM"))
    {
        ...
        { // 256 KB Action Replay 3 ROM
            ...
            PrepareBootUpload(0x40, 0x00, 0x04, 0x00);
			SendFile(&romfile);
