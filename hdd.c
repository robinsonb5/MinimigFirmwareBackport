...
{ // builds Identify Device struct
	...
	switch(hdf[unit].type)
	{
		...
		case HDF_CARDPART3:
			...
			memcpy((char*)&pBuffer[10], "MiniMigSD0", 10); // serial number - byte swapped
#else
...
//    unsigned long sptt[] = { 63, 127, 255, -1 };
    unsigned long sptt[] = { 63, 127, 255, 0 }; // modifikate from MiST
...
//    for (i = 0; sptt[i] >= 0; i++)
    for (i = 0; sptt[i] != 0; i++)  // modifikate from MiST
    {
        spt = sptt[i];
