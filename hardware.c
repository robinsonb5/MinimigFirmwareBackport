...
void __init_hardware(void)
{
    ...
    *AT91C_PIOA_SODR = MMC_SEL | MMC_CLKEN | FPGA0 | FPGA1 | FPGA2; // set output data register

    // output enable register
    *AT91C_PIOA_OER = DISKLED | MMC_SEL | FPGA0 | FPGA1 | FPGA2;
    // pull-up disable register
    *AT91C_PIOA_PPUDR = DISKLED | MMC_SEL | MMC_CLKEN | FPGA0 | FPGA1 | FPGA2;
...
unsigned long CheckTimer(unsigned long time)
{
    unsigned long systimer = (*AT91C_PITC_PIIR & AT91C_PITC_PICNT);
    time -= systimer;
    return(time > (1UL << 31));	//Modificate from MiST
}
...
RAMFUNC void SPI_block_read(unsigned char *pReadBuffer) {
