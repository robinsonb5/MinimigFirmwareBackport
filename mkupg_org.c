...
const unsigned long crc32_table[256] = { 0x00000000, 0x77073096, 0xEE0E612C,
...
static char SIGN[] = { "MNMGUPG" };

static char filename[] = { "firmware.bin" };
...
int main(int argc, char *argv[]) {
   ...
   c = ftell(ptr); // Filesize
   fseek(ptr, 0L, SEEK_SET);

   buffer = malloc(c);
   fread(buffer, c, 1, ptr);
   fclose(ptr);

   memcpy(pUpgrade->id, SIGN, sizeof(SIGN));
   pUpgrade->rom.size = c;
   pUpgrade->rom.crc = ~CalculateCRC32(-1, buffer, c);;
   pUpgrade->rom.size = c;
   pUpgrade->crc = ~CalculateCRC32(-1, sector_buffer, 512 - 4);
	printf("ROM size              : %d\n", pUpgrade->rom.size);
	printf("ROM CRC               : %08X\n", pUpgrade->rom.crc);
	printf("Header CRC            : %08X\n", pUpgrade->crc);
	//printf("Version string        : %s\n", pUpgrade.version);
   firmware = malloc(c + 512);
   memset(firmware, 0, c + 512);
   memcpy(&firmware[512], buffer, c);
   memcpy(&firmware[0], &sector_buffer[0], 512);
   binaryfile = fopen("firmware.upg", "wb");
   fwrite(firmware, c + 512, 1, binaryfile);
   fclose(binaryfile);
}
